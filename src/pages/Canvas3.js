...
const handleSaveLayout = async () => {
  try {
    ...
    localStorage.setItem('id', JSON.stringify(response[0]._id));
  } catch (error) {
    console.error('Failed to save layout:', error);
  }
};
const updateLayout=async ()=>{debugger
    try {
        const storedId = localStorage.getItem('id');
      if(storedId){
        const id = storedId?JSON.parse(storedId):'';
        const layouts = await layoutService.updateLayout(id,{layoutItems:layout[0]});
      }
     } catch (error) {
